...
class _MisRutasState extends State<MisRutas> {
  ...
  Future<void> _cargarDirecciones(Map<String, dynamic> data) async {
  ...
  Widget build(BuildContext context) {
    ...
    if (clave == null) {
      return const Scaffold(
        body: Center(child: Text('No se encontró sesión activa')),
      );
    }

    final rutaStream = FirebaseFirestore.instance
        .collection('usuarios')
        .doc(clave)
        .collection('rutas')
        .doc('info')
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          if (!snapshot.hasData || !snapshot.data!.exists) {
            return const Center(child: Text("No tienes rutas registradas."));
          }

          final data = snapshot.data!.data() as Map<String, dynamic>;
          final nombreRuta = data['nombreRuta'] ?? 'Ruta Programada';
          final lugaresDisponibles = data['lugaresDisponibles'] ?? 0;
          final horarios = data['horarios'] as List<dynamic>?;

          String horarioTexto = 'Sin horarios registrados';
          ...
          if (direccionOrigen == null || direccionDestino == null) {
            _cargarDirecciones(data); // Solo se carga una vez
